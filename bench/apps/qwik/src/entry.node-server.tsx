 ...
 * WHAT IS THIS FILE?
 *
 * It's the entry point for the Node.js HTTP server when building for production,
 * as `npm run qwik add node-server` sets it up: the built-in `http.createServer`
 * with Qwik City's Node middleware, no framework in between.
 *
 * Learn more about Node.js server integrations here:
 * - https://qwik.dev/docs/deployments/node/
 *
 * The middleware takes the site's origin from the ORIGIN environment variable,
 * which it compares with the Origin header of form posts (its CSRF check):
