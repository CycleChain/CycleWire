...
test.describe('delegation', () => {
    test('handlers receive one context object', async ({ page }) => {
        await boot(page, { html: '<button id="b" data-cw-action="log#context">go</button>' });
        await page.click('#b');
        await expectLog(page, [{ fn: 'context', keys: ['action', 'element', 'event', 'props', 'signal', 'target', 'wire'], aborted: false, isWire: true }]);
    });

    test('data-cw-props reaches the handler as parsed JSON', async ({ page }) => {
        await boot(page, { html: `<button id="b" data-cw-action="log" data-cw-props='{"id": 42, "tags": ["a"]}'>go</button>` });
        await page.click('#b');
    ...
    test('the innermost bound element wins and the target is captured', async ({ page }) => {
        await boot(page, {
            html: '<div id="card" data-cw-action="log#second" tabindex="0"><button id="inner" data-cw-action="log"><b id="label">go</b></button></div>',
        });
        await page.click('#label');
        await expectLog(page, [{ fn: 'run', el: 'inner', type: 'click', target: 'label', props: null }]);
    });

    test('data-cw-on-<event> binds any delegated event, alongside the shorthand', async ({ page }) => {
        await boot(page, { html: '<button id="b" data-cw-action="log" data-cw-on-keydown="log#second">go</button>' });
        await page.focus('#b');
    ...
    test('listen() delegates extra event types', async ({ page }) => {
        await boot(page, { html: '<button id="b" data-cw-on-dblclick="log">go</button>' });
    ...
    test('a handler that stops propagation below the document hides the event, unless CycleWire captures', async ({ page }) => {
        ...
        const html = '<div id="island"><button id="b" data-cw-action="log">go</button></div>';
    ...
    test('links are never prevented unless asked, so they still navigate', async ({ page }) => {
        await boot(page, { html: '<a id="a" href="/fixtures/?build=esm-dev#next" data-cw-action="log">next</a>' });
    ...
    test('target=_blank links open and still run their action', async ({ page, context }) => {
        await boot(page, { html: '<a id="a" href="/fixtures/index.html#popup" target="_blank" data-cw-action="log">site</a>' });
    ...
    test('href="#" links are prevented', async ({ page }) => {
        await boot(page, { html: '<a id="a" href="#" data-cw-action="log">menu</a>' });
    ...
    test('a prevented link still opens in a new tab on modified clicks, without running', async ({ page, context }) => {
        await boot(page, { html: '<a id="a" href="/fixtures/index.html#tab" data-cw-action="log" data-cw-prevent>tab</a>' });
    ...
    test('data-cw-ignore stops the search and disabled elements swallow events', async ({ page }) => {
        await boot(page, {
            html: `<div data-cw-action="log#second" id="outer" tabindex="0">
                     <div data-cw-ignore><span id="ugc">user content</span></div>
                     <span id="plain">plain</span>
                     <button id="off" data-cw-action="log" disabled>off</button>
                     <span id="aria" role="button" tabindex="0" data-cw-action="log" aria-disabled="true">aria</span>
                   </div>`,
        });
    ...
    test('cancelling cw:run lets the browser default through', async ({ page }) => {
        await boot(page, { html: '<a id="a" href="#" data-cw-action="log">x</a>' });
    ...
    test('cw:done and cw:error bubble to the document; onError replaces console.error', async ({ page }) => {
        await boot(page, { start: false, html: '<button id="ok" data-cw-action="log">ok</button><button id="bad" data-cw-action="log#boom">bad</button>' });
        await page.evaluate(() => {
            window.__events = [];
    ...
    test('handlers with positional parameters draw a development warning', async ({ page }) => {
        await boot(page, { html: '<button id="b" data-cw-action="log#positional">x</button>' });
        await page.click('#b');
    ...
    test('stop() removes every listener and start() brings them back', async ({ page }) => {
        await boot(page, { html: '<button id="b" data-cw-action="log">x</button>' });
    ...
    test('the <details> shorthand runs on toggle, a non-bubbling event', async ({ page }) => {
        await boot(page, { html: '<details id="d" data-cw-action="log"><summary id="s">more</summary>body</details>' });
        await page.click('#s');
        await expect.poll(async () => (await log(page)).map((entry) => `${entry.el}:${entry.type}`)).toEqual(['d:toggle']);
    });

    test('Invoker Commands: data-cw-on-command receives custom --commands', async ({ page }) => {
        await boot(page, {
            html: `<button id="invoker" commandfor="panel" command="--refresh">refresh</button>
                   <div id="panel" data-cw-on-command="log#command"></div>`,
        });
