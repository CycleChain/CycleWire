...
test.describe('cyclewire/signals', () => {
    test('nothing is rendered on load; state comes alive when touched', async ({ page }) => {
        ...
        await bootSignals(page, `
            <section data-cw-state='{"count": 2, "open": false}'>
                <output id="count" data-cw-bind="text: count">server</output>
                <div id="panel" data-cw-bind="show: open; attr.aria-expanded: open">panel</div>
                <button id="inc" data-cw-action="state#inc">+1</button>
                <button id="toggle" data-cw-action="state#toggle">toggle</button>
            </section>`);
    ...
    test('two-way bindings wake the scope on first input', async ({ page }) => {
        await bootSignals(page, `
            <form data-cw-state='{"query": "", "done": false, "color": "red", "size": 1}'>
                <input id="q" data-cw-bind="value: query">
                <p id="echo" data-cw-bind="text: query; class.empty: !query"></p>
                <input id="done" type="checkbox" data-cw-bind="checked: done">
                <p id="status" data-cw-bind="class.is-done: done">status</p>
                <input type="radio" name="c" id="red" value="red" data-cw-bind="value: color" checked>
                <input type="radio" name="c" id="blue" value="blue" data-cw-bind="value: color">
                <p id="color" data-cw-bind="text: color; style.color: color"></p>
                <input id="size" type="number" data-cw-bind="value: size">
                <p id="double" data-cw-bind="text: size"></p>
            </form>`);
    ...
    test('named stores reach every bound element and late content binds on arrival', async ({ page }) => {
        await bootSignals(page, `
            ...
            <header><span id="badge" data-cw-bind="text: $cart.count">0</span></header>
            <button id="add" data-cw-action="state#add" data-cw-props='{"sku": "w1", "price": 12}'>Add</button>
            <p id="total" data-cw-bind="text: $cart.total">0</p>
            <div id="late"></div>`);
        ...
        await page.evaluate(() => {
            document.getElementById('late').innerHTML = '<span id="mini" data-cw-bind="text: $cart.count">?</span>';
        });
    ...
    test('scopes can seed from a JSON script by id, and nested scopes stay separate', async ({ page }) => {
        await bootSignals(page, `
            ...
            <section data-cw-state="#seed">
                <output id="outer" data-cw-bind="text: count">10</output>
                <button id="outer-inc" data-cw-action="state#inc">+</button>
                <div data-cw-state='{"count": 0}'>
                    <output id="inner" data-cw-bind="text: count">0</output>
                    <button id="inner-inc" data-cw-action="state#inc">+</button>
                </div>
            </section>`);
    ...
    test('bindings stop following elements removed from the page', async ({ page }) => {
        await bootSignals(page, `
            <section id="s" data-cw-state='{"count": 0}'>
                <output id="o" data-cw-bind="text: count">0</output>
                <button id="inc" data-cw-action="state#inc">+</button>
            </section>`);
        await page.click('#inc');
