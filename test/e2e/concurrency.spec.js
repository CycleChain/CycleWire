...
test.describe('concurrency', () => {
    test('drop (click default): a second click while running is ignored, nothing is aborted', async ({ page }) => {
        await boot(page, { html: '<button id="b" cw-action="gated">go</button>' });
        await page.click('#b');
        await waitForLog(page, '"start"');
        await expect(page.locator('#b')).toHaveAttribute('cw-pending', '');
        await expect(page.locator('#b')).toHaveAttribute('aria-busy', 'true');
        await page.click('#b');
        await page.waitForTimeout(100);
        await open(page, 'b');
        await expectLog(page, [['start', 'b', 'click'], ['end', 'b']]);
        await expect(page.locator('#b')).not.toHaveAttribute('cw-pending');
    ...
    test('restart (input default): new input aborts the stale run and runs', async ({ page }) => {
        await boot(page, { html: '<input id="i" cw-action="gated">' });
        await page.focus('#i');
    ...
    test('latest (change default): runs the last event after the current run', async ({ page }) => {
        await boot(page, { html: '<select id="s" cw-action="gated"><option>a</option><option>b</option><option>c</option></select>' });
    ...
    test('parallel: every event starts its own run', async ({ page }) => {
        await boot(page, { html: '<button id="p" cw-action="gated" cw-concurrency="parallel">go</button>' });
        await page.click('#p');
        await page.click('#p');
    ...
    test('once: counts only a successful run, then consumes further events', async ({ page }) => {
        await boot(page, { html: '<a id="o" href="#" cw-action="gated#flaky" cw-once>once</a>' });
    ...
    test('debounce: only the last event in a burst runs', async ({ page }) => {
        await boot(page, { html: '<input id="i" cw-action="log#value" cw-debounce="400">' });
        await page.focus('#i');
