...
| Import | Adds | brotli |
| --- | --- | --- |
| `cyclewire` | Activation: everything in the next section | 4.6 kB |
| `cyclewire/css` | Stylesheets that arrive with the actions that need them | 0.6 kB |
| `cyclewire/dom` | Safe `html` templates, inert fragments, `swap`, `transition` | 1.9 kB |
| `cyclewire/morph` | Morphing to new server HTML while keeping focus and input | 1.8 kB |
| `cyclewire/signals` | Signals, reactive stores, `data-cw-bind` resumed from server JSON | 2.9 kB |
...
For prototypes and small sites, `cyclewire.full.global.min.js` (12.2 kB) bundles every
